...
function tomosElegidos(cantidadTomos) {
  ...
  for (let i = 1; i <= Number(cantidadTomos); i++)
...
function calcularDescuentov2(cantidadTomos) {
  if (Number(cantidadTomos) >= 5 && Number(cantidadTomos) <= 10) {
    ...
    descuento = 20;
  } else if (Number(cantidadTomos) > 15 && Number(cantidadTomos) <= 23) {
    descuento = 40;
  } else if (Number(cantidadTomos) > 0 && Number(cantidadTomos) < 5) {
    descuento = 0;
...
function comprarColeccion() {
  ...
  if (cantidadTomos != null) {
    ...
    if (confirmarCompra()) {
      calcularDescuentov2(Number(cantidadTomos));
    }
  }
}
